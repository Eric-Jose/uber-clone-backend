...
router.post('/accept', async (req, res) => {
  ...
  try {
    ...
    const accepted = {
      ...latestRide,
      driverId,
      driverName: driver.name || driver.email || 'Motorista',
      driverProfilePhoto: driver.profilePhoto || null,
      driverLocation: driver.currentLocation || null,
      passengerLocation: latestRide.passengerLocation || latestRide.origin?.location || null,
      status: 'ACCEPTED',
    ...
    acceptingRideIds.delete(lockKey);
  }
});
async function updateRideStatus(req, res) { try { const id = req.params.rideId || req.body.rideId, { status, cancellationReason } = req.body; if (!id || !VALID_STATUSES.includes(status)) return res.status(400).json({ error: 'Status de corrida inválido.' }); const ref = db.ref(`rides/${id}`), snapshot = await ref.get(), ride = snapshot.val(); if (!ride) return res.status(404).json({ error: 'Corrida não encontrada.' }); const uid = req.user.uid; if (ride.userId !== uid && ride.driverId !== uid) return res.status(403).json({ error: 'Acesso negado.' }); const allowedTransitions = { SEARCHING: ['CANCELLED'], ACCEPTED: ['IN_PROGRESS','CANCELLED'], IN_PROGRESS: ['COMPLETED','CANCELLED'], COMPLETED: [], CANCELLED: [] }; if (!allowedTransitions[ride.status]?.includes(status)) return res.status(409).json({ error: `Transição inválida: ${ride.status} → ${status}.` }); if (status === 'IN_PROGRESS' && ride.driverId !== uid) return res.status(403).json({ error: 'Somente o motorista pode iniciar a corrida.' }); if (status === 'COMPLETED' && ride.driverId !== uid) return res.status(403).json({ error: 'Somente o motorista pode finalizar a corrida.' }); const update = { status, updatedAt: admin.database.ServerValue.TIMESTAMP }; if (status === 'CANCELLED') { update.cancelledBy = uid; if (cancellationReason) update.cancellationReason = String(cancellationReason).slice(0, 200); } await ref.update(update); const updated = (await ref.get()).val(), payload = { rideId: id, ride: updated }; if (status === 'IN_PROGRESS') emitToRide(id, 'ride-started', payload); if (status === 'COMPLETED') emitToRide(id, 'ride-ended', payload); if (status === 'CANCELLED') emitToRide(id, 'ride-cancelled', { ...payload, cancelledBy: uid, cancellationReason: updated?.cancellationReason || null }); return res.json({ success: true, status, ride: updated }); } catch (error) { console.error('Erro ao atualizar status:', error); return res.status(500).json({ error: 'Erro ao atualizar status da corrida.' }); } }
